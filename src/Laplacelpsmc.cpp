...
List Rcpp_Laplace(NumericVector lat0, double v, int K, Function PDcorrect,
  ...
  while(dist > tol){
    ...
    checkPD = PDcorrect(Precmat);
    condcheck = checkPD["isPD"];
    PDpassed  = is_true(all(condcheck));
    if(PDpassed == FALSE){
      Precmat = as<arma::mat>(checkPD[0]);
      pdadj = pdadj+1;
    }
  ...
  // Indicate a note for positive definiteness adjustment
  if(pdadj == 0){
   PDnote = "No PD adjustment";
  } else{
   PDnote = "PD has been adjusted";
  }
